...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: FormEvent) => {
    e.preventDefault();
    try {
      const response = await axios.post("/Account/login", loginForm);

      console.log("Login successful!", response.data);
      // // Save the token to local storage or context for future requests
      ...
      toast.success(
        ...
        {
          position: "top-center",
          // className: " w-16 bg-red-700 h-100  ",
        }
      );
    } catch (error) {
        ...
        <div className="m-auto py-12 px-6 sm:p-20 xl:w-10/12">
          ...
          <form onSubmit={handleLogin} className="space-y-6 py-6">
            <div>
              <Input
                prefix={
                  <IoMailOutline className="  text-[28px] text-gray-400 " />
                }
            ...
            <div className="flex flex-col items-end">
              <Input
                prefix={
                  <RiLockPasswordLine className="  text-[28px] text-gray-400 " />
                }
